...
#pragma mark - Liquid Glass 高光边（通用 UIKit，iOS 13+ 安全，无 iOS 26 新 API）

// 液态玻璃标志性的「玻璃上缘高光」：容器顶部内侧一条渐隐的白色亮边。
// 纯 UIView + CAGradientLayer 实现，不依赖任何 iOS 26 专属 API，
// 老系统（iOS < 26）完全安全 —— 只是视觉增强，无新符号/新框架。
static const NSInteger kGlassHighlightTag = 99994;

- (UIView *)glassHighlightViewForView:(UIView *)view {
    if (!view) return nil;
    for (UIView *sub in view.subviews) {
        if (sub.tag == kGlassHighlightTag) return sub;
    }
    CGFloat h = 1.5;
    UIView *hl = [[UIView alloc] initWithFrame:CGRectMake(0, 0, view.bounds.size.width, h)];
    hl.tag = kGlassHighlightTag;
    hl.userInteractionEnabled = NO;
    hl.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleBottomMargin;
    CAGradientLayer *g = [CAGradientLayer layer];
    g.frame = hl.bounds;
    // 液态玻璃上缘光：中间稍亮、两端渐隐（模拟玻璃受光后的边缘高光）
    g.colors = @[
...
// 给任意容器加液态玻璃高光边（幂等：重复调用只保留一条）。
// 仅当 UI 效果为毛玻璃模式时添加（半透明模式不适用）。
//
// 版本分级（用户策略；注意 iOS 19~25 不存在，苹果 2025 年从 18 直接跳 26）：
//   - iOS 26+  ：系统自带原生液态玻璃（Liquid Glass），手动高光会画蛇添足，
//     直接 return 不做任何干预，让系统 UI 材质生效。
//   - iOS 18   ：唯一夹在中间的系统版本，无系统液态玻璃，
//     用手动高光边模拟玻璃质感（纯 UIKit）。
//   - iOS < 18 ：走原 UI 样式，不做任何改动（老设备兼容优先）。
// #available 是标准运行时检查，旧系统自动走对应分支，
// 不会因 iOS 26 SDK 符号在老系统上报错（编译期 availability 保护）。
- (void)applyGlassHighlightToView:(UIView *)view {
    if (!view) return;
    if (@available(iOS 26.0, *)) {
        // iOS 26+：系统原生液态玻璃接管，不叠加手动装饰
        return;
    }
    if (!@available(iOS 18.0, *)) {
        // iOS < 18：原 UI 样式，不添加手动高光
        return;
    }
    if (self.uiEffect != BackgroundUIEffectBlur) return;
    if ([view viewWithTag:kGlassHighlightTag]) return;
    [self glassHighlightViewForView:view];
}

// 移除高光边（切回半透明模式 / 关闭液态玻璃观感时调用）。
- (void)removeGlassHighlightFromView:(UIView *)view {
    if (!view) return;
    UIView *hl = [view viewWithTag:kGlassHighlightTag];
    if (hl) [hl removeFromSuperview];
}
